...
from report_lib.text import gen_summary_line, output_results_text
from report_lib.util import create_cov_summary_dict, create_test_summary_dict
from test_run_result import Failure_Modes, TestRunResult
...
def main() -> int:
    ...
    with LockedMetadata(args.dir_metadata, __file__) as md:
        ...
        for f in md.tests_pickle_files:
            try:
                ...
                summary_dict[f"{trr.testname}.{trr.seed}"] = (
                    ...
                    + (" {T}" if (trr.failure_mode == Failure_Modes.TIMEOUT) else "")
                )
                if trr.passed:
        ...
        with (
            open(md.regr_log_junit, "w", encoding="UTF-8") as junit_xml,
            open(md.regr_log_junit_merged, "w", encoding="UTF-8") as junit_merged_xml,
        ):
            output_run_results_junit_xml(
                passing_tests, failing_tests, junit_xml, junit_merged_xml
            )

        with open(md.regr_log, "w", encoding="UTF-8") as outfile:
            #  Write results as regr.log (custom logfile format)
            output_results_text(passing_tests, failing_tests, summary_dict, outfile)

        test_summary_dict = create_test_summary_dict(passing_tests + failing_tests)
        ...
        with open(json_report_filename, "w") as json_report_file:
            output_results_dvsim_json(
                md, test_summary_dict, cov_summary_dict, json_report_file
            )
        ...
        if SVG_MODULE_PRESENT:
            ...
            with open(svg_summary_filename, "w") as svg_summary_file:
                output_results_svg(
                    test_summary_dict, cov_summary_dict, svg_summary_file
                )
        else:
        ...
        plots_dir = md.dir_run / "plots"
        output_results_matplotlib(all_tests, plots_dir)
