...
def select_coverage_paths(runfile: pathlib.Path) -> List[pathlib.Path]:
    """Return absolute coverage directories in the order tests were merged.

    Parses a coverage runfile (list of coverage database paths) and returns
    the paths in order, filtering out functional coverage paths.

    Args:
        runfile: Path to coverage runfile (one path per line)

    Returns:
        List of coverage database paths in order
    """
...
def merge_cov_vcs(md: RegressionMetadata, cov_dirs: Set[pathlib.Path]) -> int:
    cmd = [
        ...
        "both",
        "-dbname",
        str(md.dir_cov / "merged.vdb"),
        "-report",
        str(md.dir_cov / "report"),
        "-log",
        str(md.dir_cov / "merge.log"),
        "-dir",
    ...
    with LockedMetadata(md.dir_metadata, __file__) as md:
        ...
        md.cov_merge_cmds = [
            (
                ...
                + [
                    "-exec",
                    str(xcelium_scripts / "cov_merge.tcl"),
                    "-logfile",
                    str(md.dir_cov / "merge.log"),
                ]
            )
        ...
        md.cov_report_cmds = [
            (
                ...
                + [
                    "-load",
                    str(md.dir_cov_merged),
                    "-init",
                    str(md.ibex_dv_root / "waivers" / "coverage_waivers_xlm.tcl"),
                    "-exec",
                    str(xcelium_scripts / "cov_report.tcl"),
                    "-logfile",
                    str(md.dir_cov / "report.log"),
                ]
            )
        ]
...
def main():
    ...
    if md.simulator not in ["xlm", "vcs"]:
        raise ValueError(
            f"Unsupported simulator for merging coverage: {args.simulator}"
        )
    ...
    merge_funs = {"vcs": merge_cov_vcs, "xlm": merge_cov_xlm}
